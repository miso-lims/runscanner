...
public class DefaultPacBio extends RunProcessor {
  public static Builder provider() {
    return new Builder(Platform.PACBIO, "default", DefaultPacBio::create);
  }
  ...
  /**
   * Create a metadata defaultprocessors to populate the map of wellname → poolbarcodes in the DTO.
   *
   * @return
   */
  ...
  protected String getSampleSheet(String url) {
    try {
      return HTTP_CLIENT.execute(
          new HttpGet(url), response -> new Scanner(response.getEntity().getContent()).next("\\Z"));
    } catch (IOException e) {
  ...
  protected StatusResponse getStatus(String url) {
    try {
      return HTTP_CLIENT.execute(
          new HttpGet(url),
          response -> MAPPER.readValue(response.getEntity().getContent(), StatusResponse.class));
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
  }
  ...
  public NotificationDto process(File runDirectory, TimeZone tz) throws IOException {
    ...
    if (dto.getHealthType().isDone()) {
      toDate =
          dto.getCompletionDate().plusDays(1).truncatedTo(ChronoUnit.DAYS).format(URL_DATE_FORMAT);
    } else {
    ...
    var url =
        Stream.of(
                new Pair<>("instrument", dto.getSequencerName()),
                new Pair<>("run", dto.getRunAlias()),
                new Pair<>(
                    "from",
                    dto.getStartDate().truncatedTo(ChronoUnit.DAYS).format(URL_DATE_FORMAT)),
                new Pair<>("to", toDate))
            .map(p -> p.first() + "=" + URLEncoder.encode(p.second(), StandardCharsets.UTF_8))
            .collect(Collectors.joining("&", address + "/Metrics/RSRunReport?", ""));
    dashboardMetric.put("href", url);
