    ...
    for (File analysisAttempt : Objects.requireNonNull(analysisDir.listFiles())) {
      if (analysisAttempt.isDirectory() && analysisAttempt.getName().matches(NUMERAL)) {
        ...
        if (!sampleSheet.exists()) {
          // TODO: we will have novaseqx runs with variables amount of dragen on-ness. tolerate no
          // samplesheet
          throw new IllegalStateException(
              "Can't not have a samplesheet at " + sampleSheet.getPath());
        }
        ...
        File fastqList = new File(analysisAttempt, "Data/BCLConvert/fastq/Reports/fastq_list.csv");
        if (!fastqList.exists()) { // TODO tolerate this too
          throw new IllegalStateException("Can't not have a fastq list at " + fastqList.getPath());
        }
